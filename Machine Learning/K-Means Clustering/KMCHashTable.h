...
class HashMap{
    private:
        ...
        and a vector (value/data), which in turn contains a vector of ints (x,y,z,n coordinates)
        */
        list<pair<int, vector<vector<int>>>> table[CAPACITY]; 

    public:

        bool isEmpty() const;
        int hashFunction(int key);
        void insertItem(int key, vector<vector<int>> data);
        void removeItem(int key);
        vector<vector<int>> findItem(int key);
...
void HashMap::insertItem(int key, vector<vector<int>> data){
    ...
    if(!keyExists){
        //key, data
        pair<int, vector<vector<int>>> tempPair;
        tempPair.first = key;
...
vector<vector<int>> HashMap::findItem(int key){
    int hashValue = hashFunction(key);
    auto& cell = table[hashValue];
    auto iterator = begin(cell);
...
void HashMap::printTable(){
    for (int i{}; i < CAPACITY; i ++){
        ...
        for (; iterator != table[i].end(); iterator ++){ // for every classification
            cout << "Key: " << iterator -> first << endl;
            cout << "Values: " << endl;
            for(int j{}; j < iterator -> second.size(); j ++){ // for every vector (point) assigned within this classification
                cout << "(";
                for(int k{}; k < iterator -> second[0].size(); k ++){ // for every coordinate point in this vector
                    cout << (iterator -> second)[j][k] << ",";
                } 
                cout << ")" << endl;
